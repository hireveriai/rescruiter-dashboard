...
export function openWarRoom() {
  ...
  if (!token) {
    window.alert("Unable to open War Room because no auth token cookie is available.");
    return false;
  }

  const payload = decodeJwtPayload(token);
  const orgId = readOrgIdFromPayload(payload);

  if (!orgId) {
    window.alert("Unable to open War Room because orgId is missing from the auth token cookie.");
    return false;
  }
