-- Supabase dashboard-safe install script.
--
-- Run Block 1 first, then Block 2. Do not select only part of a block.
-- The error "unterminated dollar-quoted string" means the dashboard ran only
-- the opening part of a CREATE FUNCTION statement.

-- =========================
-- Block 1: Recursive helper
-- =========================
...
begin
  ...
  then
    if p_dry_run then
      insert into pg_temp.hv_delete_organization_report
        (depth, relation_name, affected_rows, action)
      select
        0,
        'public.auth_users',
        count(*)::bigint,
